...
from pipe.utils import (
    cascadate, mapf, filterf, add, sub, multiply, divide, reverse_args
)
...
def test_pipe_2():
    pipe = Pipe()
    pipe >> 1 >> add(1) >> multiply(2) >> reverse_args(sub)(1) >> reverse_args(divide)(3) >> cascadate(lambda x: round(x)) >> add(3) >> hey  # noqa: E501
...
def test_pipe_3():
    pipe = Pipe()
    pipe >> [1, 2, 3] >> mapf(add(1)) >> filterf(is_even) >> filterf(cascadate(lambda x: x > 3))  # noqa: E501
...
# def test_pipe_4():
#     pipe = Pipe()
#     pipe >> 2 >> iff(is_even)('even')('odd')
#     assert pipe.result == 'even'
#
#     pipe = Pipe()
#     pipe >> 3 >> iff(is_even)('even')('odd')
